...
df['성별'] = df['성별'].str.upper().fillna('U') # U : 성별알수없음의 뜻
print( df.isnull().sum() )  # 결측 재확인   # 성별 결측치 : 0

# (7) 수치형 문자형 구분하기 위해 df.info() 결과 확인 , 
...
# df.select_dtypes( inclue = '선택할타입' ) : 데이터타입으로 데이터 선택
# number_data = df.select_dtypes( include= ['이용시간' , '이용거리' , '시간대' ] ) 

#number_data = df.select_dtypes( include='number' )  # [자동]
number_data = df.select_dtypes( include='number' )
matrix = number_data.corr()     # 상관계수 생성
sns.heatmap( matrix , cmap='coolwarm' , annot=True )    # annot=True 해야 수치 보임
plt.show()    