...
plt.bar( x - 0.2 , korean , width= 0.2 , label = '국어성적' , color = 'aqua' )
plt.bar( x + 0.2 , math , width= 0.2 , label = '수학성적' , color = 'yellow' )
plt.title('성적 비교')
plt.xlabel('학생')
plt.ylabel('점수')
plt.legend()
...
explode = [ 0.1 , 0 , 0 , 0 ]
plt.pie( sizes , labels=labels , colors=colors , explode=explode , startangle=90 , autopct='%0.1f%%')
plt.title('전자제품 선호도')
plt.legend()
plt.show()
...
plt.hist( data , color='lightblue' , alpha = 0.5 , bins= 5 )
plt.show()
...
# 5행 4열의 리스트 데이터 
# [[50, 60, 70, 80], [10, 20, 30, 40], [90, 80, 70, 60], [30, 40, 50, 60], [20, 10, 5, 0]]를 활용하여 히트맵을 그리시오.
